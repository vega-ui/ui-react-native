import * as React from 'react';
import Svg, { Mask, Path, G } from 'react-native-svg';
import type { SvgProps } from 'react-native-svg';
const SvgMo = (props: SvgProps) => (
  <Svg fill="none" viewBox="0 0 32 24" width={24} height={24} {...props}>
    <Mask
      id="MO_svg__a"
      width={32}
      height={24}
      x={0}
      y={0}
      maskUnits="userSpaceOnUse"
    >
      <Path fill="#fff" d="M0 0h32v24H0z" />
    </Mask>
    <G mask="url(#MO_svg__a)">
      <Path
        fill="#1C9975"
        fillRule="evenodd"
        d="M0 0v24h32V0H0z"
        clipRule="evenodd"
      />
      <Mask
        id="MO_svg__b"
        width={32}
        height={24}
        x={0}
        y={0}
        maskUnits="userSpaceOnUse"
      >
        <Path
          fill="#fff"
          fillRule="evenodd"
          d="M0 0v24h32V0H0z"
          clipRule="evenodd"
        />
      </Mask>
      <G mask="url(#MO_svg__b)">
        <Path
          fill="#FECA00"
          fillRule="evenodd"
          d="M16.022 6.854l-1.634.986.373-1.925-1.361-1.44 1.842-.078.78-1.797.779 1.797h1.839l-1.357 1.518.408 1.925-1.67-.986zm-4.639.73l-1.01.61.23-1.19-.841-.89 1.139-.049.482-1.11.482 1.11h1.137l-.84.94.253 1.19-1.032-.61zm9.197 0l-1.01.61.231-1.19-.841-.89 1.139-.049.482-1.11.482 1.11H22.2l-.84.94.253 1.19-1.032-.61zM8.766 9.61l-.81.49.185-.955-.675-.715.914-.038.386-.892.387.892h.913l-.674.753.203.955-.829-.49zm14.794 0l-.81.49.185-.955-.675-.715.914-.038.387-.892.387.892h.912l-.674.753.203.955-.828-.49z"
          clipRule="evenodd"
        />
        <Mask
          id="MO_svg__c"
          width={16}
          height={16}
          x={8}
          y={6}
          maskUnits="userSpaceOnUse"
        >
          <Path
            fill="#fff"
            fillRule="evenodd"
            d="M16 22a8 8 0 100-16 8 8 0 000 16z"
            clipRule="evenodd"
          />
        </Mask>
        <G fill="#F7FCFF" mask="url(#MO_svg__c)">
          <Path
            fillRule="evenodd"
            d="M12.68 21.064h6.64v1.072h-6.64v-1.072zM9.957 19.46h12.766v.983H9.957v-.983z"
            clipRule="evenodd"
          />
          <Path d="M12.62 19.02H8.716v-1h3.897c1.381-.019 2.39-.413 3.063-1.172l.336-.378.374.34c.899.815 1.797 1.21 2.706 1.21h4.304v1H19.09c-1.043 0-2.05-.392-3.014-1.16-.854.756-2.012 1.14-3.457 1.16z" />
        </G>
        <Path
          fill="#F7FCFF"
          fillRule="evenodd"
          d="M16.035 7.608s-1.71 1.688-1.71 4.004c0 .496.08.964.202 1.394-.55-1.22-.588-2.543-.113-3.986l-.647-.213c-.483 1.468-.486 2.849-.006 4.128a8.212 8.212 0 00-3.306-1.13l-.075.677a7.52 7.52 0 012.902.96 5.394 5.394 0 00-.681-.223c-2.163-.552-4.285.23-4.285.23s1.224 2.747 3.595 3.152c.118.02.235.036.35.047h-.018c-1.13 0-2.181-.462-3.166-1.401l-.47.493c1.106 1.054 2.322 1.589 3.636 1.589 1.285 0 2.48-.511 3.57-1.52.175-.106.277-.182.293-.194l.043.037-.033.06s.107.064.301.156c1.077.835 2.424 1.461 3.49 1.461 1.315 0 2.53-.535 3.636-1.59l-.47-.492c-.984.939-2.035 1.401-3.166 1.401-.413 0-.817-.066-1.214-.2.556.03 1.168-.02 1.802-.214 2.335-.714 3.309-2.873 3.309-2.873s-1.243-.67-2.821-.678a7.904 7.904 0 011.107-.2l-.074-.678c-1.48.164-2.761.664-3.836 1.497.628-1.38.677-2.885.147-4.495l-.647.213c.433 1.316.44 2.531.021 3.66.072-.334.114-.69.114-1.068 0-2.442-1.78-4.004-1.78-4.004zm.751 7.147l-.025.037.031-.037h-.006zm-1.651-.159l.067-.062c.06.101.121.197.18.285a7.58 7.58 0 00-.247-.223z"
          clipRule="evenodd"
        />
      </G>
    </G>
  </Svg>
);
export default SvgMo;
